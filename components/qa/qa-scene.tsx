      ...
      <AnimatePresence>
        {state.showSubtitle && (
          <motion.p
            ...
            className="mb-3 max-w-[70ch] text-lg leading-snug text-slate-600"
          >
        ...
        {state.showComparison && (
          ...
          >
            <div className="flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50/85 px-5 py-3 shadow-sm backdrop-blur">
              ...
              <span className="font-display text-4xl font-bold text-red-700">
                48 lost
              </span>
            </div>
            <motion.span
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/85 px-5 py-3 shadow-sm backdrop-blur">
              <span className="font-mono text-xs uppercase tracking-[0.25em] text-emerald-700">
                Risk-weighted
              </span>
...
function ProductionLine({
  ...
  const UNIT_W = 28; // matches ProductShape W
  const UNIT_GAP = 12; // tailwind gap-3
  const TRACK_PAD = 16; // tailwind px-4

  return (
    <div className="flex items-center gap-3">
      {/* Machine: stamping press with animated piston + status LED */}
      <div
        className="flex h-24 w-24 shrink-0 items-center justify-center rounded-xl border shadow-sm"
        style={{
        ...
        <AnimatePresence>
          {showRiskBadge && (
            <motion.div
              ...
              className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-mono text-[11px] uppercase tracking-wider ${
                isHigh
      ...
      {/* Track of units */}
      <div className="relative flex h-20 flex-1 items-center gap-3 rounded-lg border border-slate-200/70 bg-slate-50/50 px-4">
        ...
        <AnimatePresence>
          {sampledAtUnit !== null && (
            ...
            >
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 font-mono text-[11px] uppercase tracking-wider text-slate-500">
                sample
              </div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {scrapped !== null && (
            <motion.div
              ...
              className="absolute -right-2 top-1/2 flex -translate-y-1/2 translate-x-full items-center gap-1.5 rounded-md bg-red-500 px-2.5 py-1.5 text-sm font-semibold text-white shadow-sm"
            >
              <svg
                width="10"
                height="10"
  ...
  return (
    <div className="flex h-full flex-col gap-4 rounded-2xl border border-slate-200 bg-white/70 p-5 shadow-sm backdrop-blur">
      ...
      <div>
        ...
        <div className="flex min-h-[100px] flex-wrap items-center justify-start gap-5 rounded-lg border border-dashed border-slate-200 p-3">
            ...
            <AnimatePresence initial={false}>
              {verdicts.map((v, i) => (
                ...
                >
                  ...
                  {v.passed ? (
                    <span className="flex items-center gap-1 font-mono text-sm font-bold uppercase tracking-wider text-emerald-700">
                      ✓ pass
                    </span>
                  ) : (
                    <motion.span
                      ...
                      className="flex items-center gap-1 font-mono text-sm font-bold uppercase tracking-wider text-red-700"
                    >
                      ✗ fail
                    </motion.span>
                  )}
      ...
      <AnimatePresence>
        {scrapStat !== null && (
          ...
          >
            ...
            <p className="font-display text-5xl font-bold tabular-nums text-red-700">
              {animatedStat} units
            </p>
            <p className="text-base leading-snug text-red-700/80">
...
function ProductShape({
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} fill="none">
      <motion.polygon
        points="14,2.1 25.9,15.4 25.9,40.6 14,53.9 2.1,40.6 2.1,15.4"
        strokeWidth="2"
