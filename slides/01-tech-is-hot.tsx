            ...
            {!isBadPhase ? (
              ...
              >
                {POSITIVES.slice(0, positivesShownCount).map((item, i) => (
                  <BulletItem key={i} bullet={item} dotClass="bg-brand-500" />
                ))}
              </motion.ul>
            ) : (
              <motion.ul
...
export const slide01: SlideEntry = {
  meta: {
    ...
    steps: TOTAL_STEPS,
    notes:
      "Everybody wants a tech career, and for good reasons. [click through the four upsides while naming them]\n\nBut when people hear 'tech career', they default to becoming a software developer. And that path is harder than the brochure says. [click: the slide turns dark]\n\n• Coding is genuinely difficult; most people who try it alone quit within months.\n• The market shifted: high interest rates meant fewer projects, and juniors now compete with laid-off seniors for the same roles.\n• AI is eating the entry-level tasks that juniors used to learn on.\n• Land the point: the front door to tech got narrower. The talk is about another door.",
  },
